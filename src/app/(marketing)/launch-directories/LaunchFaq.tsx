...
function FaqRow({ item, open, onToggle }: { item: Item; open: boolean; onToggle: () => void }) {
  ...
  return (
    <div className="faq-item">
      ...
      <div className="faq-answer" style={{ maxHeight: height }}>
        <div className="faq-answer-inner" ref={ref} dangerouslySetInnerHTML={{ __html: item.a }} />
      </div>
    </div>
  )
}
      ...
      <div className="wrap--sm">
        <div className="section-head rv">
          ...
          <h2 className="section-title">
            Launch questions,
            <br />
            <em>answered straight.</em>
          </h2>
        </div>

        <div className="faq-list rv">
          {launchFaqItems.map((item, i) => (
            <FaqRow
              key={i}
              item={item}
              open={open === i}
